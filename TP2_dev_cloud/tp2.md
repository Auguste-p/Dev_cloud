...
COPY package*.json ./
RUN npm install --omit=dev # Flag pour exclure devDependencies
...
# Calcul : (taille_naive - taille_multistage) / taille_naive * 100 = ____%
# Calcul valorisé : (201 - 170) / 201 * 100 = 15%
```
![Réduction taille images](images/reduction_taille_images_docker.png)

**Question :** Pourquoi les outils de build (TypeScript, gcc, etc.) ne doivent-ils **pas** être présents dans l'image de production ?
```
Réponse : On compile dans un stage build, puis on ne garde que le strict nécessaire au runtime dans l’image finale.
```
...
Réponse : On utilise condition: service_healthy parce que le conteneur PostgreSQL peut être démarré sans être encore prêt à accepter des connexions. service_started ne garantit que le processus a lancé le conteneur, pas que la base est initialisée, accessible et prête pour les requêtes.

Avec service_healthy, Docker Compose attend que le healthcheck passe avant de lancer le service web, ce qui évite les erreurs de démarrage liées à une base pas encore disponible.
```
...
# Sur Mac, j'ai dû spécifier que c'est un build Linux sinon GCP ne l'accepte pas car c'est un build ARM64 par défaut sur Macos. 
...
```
![Tests commandes](images/test_app.png)
...
# Mon URL: https://tp2-service-622615739189.europe-west9.run.app
```
...
curl https://tp2-service-zowztgcaaq-od.a.run.app/
OU
curl ${SERVICE_URL}

curl https://tp2-service-zowztgcaaq-od.a.run.app/health
OU
...
```
![Cloud GCP service actif](images/cloud_gcp_service_up.png)

**Question :** Quelle est la différence entre --max-instances=3 et --min-instances=1 dans Cloud Run?

```
Réponse : 
--max-instances=3 fixe le plafond: Cloud Run ne créera jamais plus de 3 instances, même en pic de trafic (protection coût/ressources, mais risque de latence / erreur 429 si surcharge).
--min-instances=1 fixe le plancher: Cloud Run garde au moins 1 instance toujours démarrée, même sans trafic (moins de cold start, mais coût permanent minimal).
```
...
# Réponse : Commande qui n'a pas marché pour moi, vu ensemble en cours. Plus lent, en général autour de 800 à 3000 ms.
```
```
# Combien de ms pour les appels suivants (warm)?
# Réponse : Commande qui n'a pas marché pour moi, vu ensemble en cours. Plus rapide, souvent autour de 50 à 300 ms.
```
...
```
Réponse : 
On les sépare surtout pour la sécurité et le contrôle réseau.

- Sécurité: la base de données n’a pas besoin d’être exposée à Internet, donc on la met en sous-réseau privé.
- Isolation: si la partie applicative est compromise, la base reste mieux protégée.
- Règles firewall plus fines: on autorise seulement les flux nécessaires (ex: app -> DB sur 5432).
- Bonne pratique d’architecture: séparation des rôles, plus simple à auditer et à faire évoluer.
```
...
```
![Règles firewall](images/firewall_rules.png)

**Question :** Quelle est la différence entre un Security Group (AWS) et une Firewall Rule (GCP)?
```
Réponse : 
AWS: tu raisonnes d’abord “à qui est attaché ce Security Group ?”.
GCP: tu raisonnes d’abord “quelle règle VPC s’applique à ce trafic et à quelles cibles (tags/SA) ?”.
```
...
# gcloud storage cp "gs://${BUCKET}/config.json#1775557831470979" ./config_v1.json
```
...
```
Règle 1 : supprimer les versions non actives (isLive: false) dès qu’il existe plus de 3 versions plus récentes (numNewerVersions: 3).
Cela limite l’accumulation d’anciennes versions et évite de payer du stockage inutile.

Règle 2 : pour les objets actifs (isLive: true) âgés de plus de 30 jours (age: 30), changer la classe de stockage en NEARLINE.
Cela garde les données accessibles, mais en classe “archive légère” moins chère.

Intérêt économique : NEARLINE réduit le coût de stockage des données peu consultées.
Tu paies moins cher au mois pour stocker, avec un compromis: accès/récupération un peu plus coûteux que STANDARD.
```
...
--tag=v02 # Tag pour identifier cette révision
...
```
# Résultat observé : On reçoit normalement 2 réponses du canary
# Explication mathématique (20% de 10 requêtes) : 20% de 10 ça fait 2
```
...
--format="yaml(status.traffic)"
```
![Bascule routing](images/bascule_routing_100%.png)

**Question :** Pourquoi le traffic splitting est-il préférable à un redéploiement direct (--to-latest immédiat) en production ?

```
Réponse :
Déploiement progressif: tu envoies seulement une petite part du trafic (ex: 5–20%) à la nouvelle version.
Validation réelle: tu testes avec du vrai trafic avant bascule complète.
Détection précoce: si bug/perf/régression, l’impact est limité à une fraction des utilisateurs.
Rollback rapide: tu remets le trafic sur la version stable en quelques secondes.
Meilleure continuité de service: moins de risque de panne globale qu’un --to-latest immédiat à 100%.
```
...
# Réponse : Le TTL du cache permet de ne pas saturer la mémoire et garantir la cohérence des données, sans multiplier inutilement les appels en base / API.
```
**Question :** Dans quelle situation l'utilisation d'un cache Redis peut-elle poser un **problème de cohérence** des données?

```
Réponse : Les données peuvent ne plus être cohérentes si le cache n'a pas récupérer les données depuis un moment et que l'utilisateur y accède avant la mise à jour du cache.
```
