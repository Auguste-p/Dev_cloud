...
kubectl get node # Commande pour lister les nodes
...
data:
  ...
  APP_ENV: "production"
  LOG_LEVEL: "info"
  KAFKA_BOOTSTRAP_SERVERS: "kafka-cluster-kafkabootstrap.logistream.svc.cluster.local:9092"
...
> Dans GKE, pour chiffrer les Secrets stockés dans ETCD avec une clé Cloud KMS, il faut activer l’option "Application-layer Secrets Encryption" (aussi appelée "Encryption at Rest with Customer-Managed Keys") lors de la création du cluster ou via une mise à jour. Cette configuration s’effectue avec le paramètre :

>--database-encryption-key=projects/[PROJECT_ID]/locations/[REGION]/keyRings/[KEY_RING]/cryptoKeys/[KEY_NAME]

>Cela permet à GKE de chiffrer les données sensibles (comme les Secrets Kubernetes) dans ETCD à l’aide d’une clé gérée dans Google Cloud KMS, et non plus seulement en base64.

>Pourquoi est-ce indispensable en production ?

>La base64 n’est pas un chiffrement, juste un encodage : toute personne ayant accès à ETCD peut lire les secrets.

>Le chiffrement avec Cloud KMS protège les secrets même si ETCD est compromis (ex : dump disque, accès admin non autorisé).

>Cela permet de respecter les exigences de sécurité (RGPD, ISO 27001, PCI-DSS…) et d’auditer l’accès aux clés (Cloud KMS fournit un audit trail).

>En cas de fuite, il est possible de révoquer ou faire tourner la clé KMS, ce qui invalide les anciens secrets chiffrés.

>En résumé : c’est une mesure de sécurité essentielle pour garantir la confidentialité des secrets en production et limiter l’impact d’une compromission du cluster.
...
spec:
  kafka:
    version: 3.7.0
    replicas: _______  # 3 brokers pour la haute disponibilité
    listeners:
      ...
      - name: external
        port: 9094
        type: _______  # loadbalancer (expose via un Load Balancer GCP)
        tls: false
    config:
      # Nombre de partitions par défaut pour les nouveaux topics
      num.partitions: 3
      # Facteur de réplication par défaut
      default.replication.factor: _______  # 3
...
const consumer = kafka.consumer({
  groupId: '_______',  // 'tracker-service-group'
});
...
async function startConsuming() {
  ...
  await consumer.subscribe({
    topics: ['_______', 'delivery-alerts'],  // ['truck-positions', ...]
  ...
  await consumer.run({
    ...
    eachMessage: async ({ topic, partition, message }) => {
      ...
      if (topic === 'truck-positions') {
        ...
        if (value.fuel_level < _______) {  // 20
...
- [ ] Repository Git public avec le code complet (`k8s/` + `kafka/` + `producer/` + `consumer/` + `.github/`)
...
- [ ] `README.md` avec diagramme d'architecture LogiStream (camions → GPS → Kafka → GKE → dashboard)
